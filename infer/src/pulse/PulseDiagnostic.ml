...
type information_leak = {calling_context: calling_context; trace: Trace.t; must_be_sat: PulseAbductiveDomain.Summary.t list}
[@@deriving compare, equal]
...
let pp fmt diagnostic =
  let pp_immediate fmt = F.pp_print_string fmt "immediate" in
  let rec pp_must_be_sat fmt (must_be_sat:PulseAbductiveDomain.Summary.t list) = 
