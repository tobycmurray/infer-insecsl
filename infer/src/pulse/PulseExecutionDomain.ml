...
type 'abductive_domain_t base_t =
  ...
  | InsecSLLeakageError of { astate: AbductiveDomain.Summary.t
                           ; must_be_sat: AbductiveDomain.Summary.t list
...
let pp_ pp_abductive_domain_t fmt = function
  ...
  | InsecSLLeakageError {astate;_} ->
      F.fprintf fmt "{InsecSLLeakageError %a}" AbductiveDomain.Summary.pp astate
